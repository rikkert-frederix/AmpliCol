...
contains
  ...
  subroutine ext_gluon_mass_fd(p,nhel,nsv,wf,vmass)
    ! Five-component external massive-vector wavefunction in FD gauge.
    ! Components 1:4 are the reduced vector and component 5 is the
    ! associated Goldstone mode.  With q=-p, the corresponding unitary-gauge
    ! polarization is V_U=V_FD-i*q*phi/m=V_FD+i*p*phi/m.
    implicit none
    real(kind=8),dimension(0:3),intent(in) :: p
    integer,intent(in) :: nhel,nsv
    real(kind=8),intent(in) :: vmass
    complex(kind=8),dimension(5),intent(out) :: wf
    complex(kind=8),dimension(4) :: unitary_wf
    complex(kind=8),parameter :: cImag=(0d0,1d0),cZero=(0d0,0d0)

    call ext_gluon_mass(p,nhel,nsv,unitary_wf,vmass)
    wf(1:4)=unitary_wf(1:4)
    wf(5)=cZero
    if (nhel.eq.0) then
       wf(5)=-dble(nsv)*cImag
       wf(1:4)=unitary_wf(1:4)-dble(nsv)*p(0:3)/vmass
    endif
  end subroutine ext_gluon_mass_fd

  subroutine fd_massive_to_unitary(wf,p,vmass,unitary_wf)
    ! Convert a five-component FD field to the four-component unitary field.
    implicit none
    complex(kind=8),dimension(5),intent(in) :: wf
    real(kind=8),dimension(0:3),intent(in) :: p
    real(kind=8),intent(in) :: vmass
    complex(kind=8),dimension(4),intent(out) :: unitary_wf
    complex(kind=8),parameter :: cImag=(0d0,1d0)
    unitary_wf(1:4)=wf(1:4)+cImag*p(0:3)*wf(5)/vmass
  end subroutine fd_massive_to_unitary

  subroutine fd_lift_massive_current(unitary_current,p,vmass,wf)
    ! Lift the source generated by a unitary-gauge vertex into the dual
    ! five-component FD basis.  Together with fd_massive_propagator this is
    ! an exact factorisation of the legacy unitary-gauge propagator.
    implicit none
    complex(kind=8),dimension(4),intent(in) :: unitary_current
    real(kind=8),dimension(0:3),intent(in) :: p
    real(kind=8),intent(in) :: vmass
    complex(kind=8),dimension(5),intent(out) :: wf
    complex(kind=8),parameter :: cImag=(0d0,1d0)
    complex(kind=8) :: pdotj
    pdotj=p(0)*unitary_current(1)-p(1)*unitary_current(2)-&
         p(2)*unitary_current(3)-p(3)*unitary_current(4)
    wf(1:4)=unitary_current(1:4)
    wf(5)=cImag*pdotj/vmass
  end subroutine fd_lift_massive_current

  subroutine fd_gauge_direction(q,n,nq)
    ! n(q)=(sgn(q0),-q_vec/|q_vec|), as used by the FD-gauge projector.
    implicit none
    real(kind=8),dimension(0:3),intent(in) :: q
    real(kind=8),dimension(0:3),intent(out) :: n
    real(kind=8),intent(out) :: nq
    real(kind=8) :: qabs
    qabs=sqrt(q(1)**2+q(2)**2+q(3)**2)
    n=0d0
    n(0)=sign(1d0,q(0))
    if (qabs.gt.0d0) then
       n(1:3)=-q(1:3)/qabs
    ...
    nq=n(0)*q(0)-n(1)*q(1)-n(2)*q(2)-n(3)*q(3)
  end subroutine fd_gauge_direction
  ...
  subroutine FDTwoVectorToAux(wf1,wf2,wfA)
    ! Exact 16-component non-propagating representation of the ordered
    ! four-vector contact current.  The auxiliary stores the 4x4 bilinear
    ! map which acts on the third vector at the second three-point vertex.
    implicit none
    complex(kind=8),dimension(4),intent(in) :: wf1,wf2
    complex(kind=8),dimension(16),intent(out) :: wfA
    complex(kind=8),dimension(4) :: metric_wf1,metric_wf2
    complex(kind=8) :: dot12
    integer :: mu,nu,k
    metric_wf1=[wf1(1),-wf1(2),-wf1(3),-wf1(4)]
    metric_wf2=[wf2(1),-wf2(2),-wf2(3),-wf2(4)]
    dot12=sum(wf1*metric_wf2)
    do mu=1,4
       do nu=1,4
          k=4*(mu-1)+nu
          wfA(k)=2d0*wf2(mu)*metric_wf1(nu)-wf1(mu)*metric_wf2(nu)
          if (mu.eq.nu) wfA(k)=wfA(k)-dot12
       enddo
    enddo
  end subroutine FDTwoVectorToAux

  subroutine FDAuxVectorToVector(wfA,wf3,wf)
    implicit none
    complex(kind=8),dimension(16),intent(in) :: wfA
    complex(kind=8),dimension(4),intent(in) :: wf3
    complex(kind=8),dimension(4),intent(out) :: wf
    complex(kind=8),parameter :: prefact=(0d0,0.5d0)
    integer :: mu
    do mu=1,4
       wf(mu)=prefact*sum(wfA(4*(mu-1)+1:4*mu)*wf3(1:4))
    enddo
  end subroutine FDAuxVectorToVector
  ...
  subroutine GluonPropagator_fd(wfg,p)
    ! Massless FD-gauge propagator numerator
    !   -g^{mu nu}+(q^mu n^nu+n^mu q^nu)/(n.q)
    ! in the current conventions used by this module (q=-p).
    implicit none
    complex(kind=8),dimension(1:4),intent(inout) :: wfg
    real(kind=8),dimension(0:3),intent(in) :: p
    real(kind=8),dimension(0:3) :: q,n
    real(kind=8) :: nq
    complex(kind=8) :: js1,js2,propagator
    complex(kind=8),dimension(4) :: source
    complex(kind=8),parameter :: cImag=(0d0,1d0)
    q(0:3)=-p(0:3)
    call fd_gauge_direction(q,n,nq)
    source=wfg
    js1=(n(0)*source(1)-n(1)*source(2)-n(2)*source(3)-n(3)*source(4))/nq
    js2=(q(0)*source(1)-q(1)*source(2)-q(2)*source(3)-q(3)*source(4))/nq
    wfg(1:4)=source(1:4)-q(0:3)*js1-n(0:3)*js2
    propagator=-cImag/(p(0)**2-p(1)**2-p(2)**2-p(3)**2)
    wfg(1:4)=wfg(1:4)*propagator
  end subroutine GluonPropagator_fd
  ...
  subroutine GluonPropagator_mass_fd(wfg,p,vm,vw)
    ! Five-by-five FD-gauge propagator of Hagiwara, Kanzaki and Mawatari.
    ! The fifth momentum is q(4)=-i*m and the scalar denominator is shared by
    ! all five components.
    implicit none
    complex(kind=8),dimension(1:5),intent(inout) :: wfg
    real(kind=8),dimension(0:3),intent(in) :: p
    real(kind=8),intent(in) :: vm,vw
    real(kind=8),dimension(0:3) :: q,n
    real(kind=8) :: nq
    complex(kind=8),dimension(5) :: source
    complex(kind=8) :: q5,js1,js2,propagator
    complex(kind=8),parameter :: cImag=(0d0,1d0)
    q(0:3)=-p(0:3)
    q5=-cImag*vm
    call fd_gauge_direction(q,n,nq)
    source=wfg
    js1=(n(0)*source(1)-n(1)*source(2)-n(2)*source(3)-n(3)*source(4))/nq
    js2=(q(0)*source(1)-q(1)*source(2)-q(2)*source(3)-q(3)*source(4)-&
         conjg(q5)*source(5))/nq
    wfg(1:4)=source(1:4)-q(0:3)*js1-n(0:3)*js2
    wfg(5)=source(5)-q5*js1
    propagator=-cImag/(p(0)**2-p(1)**2-p(2)**2-p(3)**2-vm**2+cImag*vm*vw)
    wfg(1:5)=wfg(1:5)*propagator
  end subroutine GluonPropagator_mass_fd
