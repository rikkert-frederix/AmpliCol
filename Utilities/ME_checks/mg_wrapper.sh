...
path_code=$(pwd)/Utilities

version=$(awk '/version/ {print $3}' $path_mg/VERSION)

cp Utilities/ME_checks/run.sh $path_mg/bin
...
if [[ "$version" == "3.6.1" ]]; then
    cp Utilities/ME_checks/matrix_standalone_v4_v361.inc $path_mg/madgraph/iolibs/template_files/matrix_standalone_v4.inc
elif [[ "$version" == "3.6.6" ]]; then
    cp Utilities/ME_checks/matrix_standalone_v4_v366.inc $path_mg/madgraph/iolibs/template_files/matrix_standalone_v4.inc
...
cp Utilities/ME_checks/color_algebra.py $path_mg/madgraph/core/
cp Utilities/ME_checks/check_sa.f $path_mg/madgraph/iolibs/template_files/check_sa.f
