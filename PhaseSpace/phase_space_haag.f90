module phase_space_haag_mod
  ...
  integer(kind=4) :: n
  real(kind=8) :: soft,drjj_min,pt_min,sqrt_smin

contains

  subroutine haag_init(this,sqrtsh,n,m,o,s_cut,pt_cut,rap_cut,dr_cut,sqrt_s_min,t_chan,include_pdf)
    implicit none
...
20    p1_cmf(0) = (s+s1-s2)/(2D0*sqrt(s))
    ...
    subroutine generate_split_Qm_Qnm(s,mass1,mass2,mn,Qm,Qnm)
      ...
      if (.not.a1_split) then
         ...
         if (flat_split) then
            ix = ix + 1
            call random_to_var(x(ix),0d0,min,max,Qz,jac)
            !soft = soft*(max-min)
         endif
    ...
    subroutine generate_first_single(k,s,s1,q1_cmf,P_cmf,a1,s2)
      ...
      if (k.gt.2) then
         ...
         elseif (flat) then
            ix = ix +1
            call random_to_var(x(ix),0d0,smax,smin,s2,jac)
            !soft = soft*(smax-smin)
            gs = 1d0
            jac = jac/gs
    ...
    subroutine generate_s2(k,s,s1,s2,q1_cmf,P_cmf)
      ...
      if (open) then
         ix = ix +1
         call random_to_var(x(ix),-1d0,smin,smax,s2,jac)
