...
!!$    if (.true.) then
    if (use_mode.eq.1.or.use_mode.eq.4) then
       ...
       pt2=p(1,next)**2+p(2,next)**2
       p(3,next)=sqrt(pt2)*sinh(y)
       p(0,next)=sqrt(pt2)*cosh(y)
    ...
    elseif(use_mode.eq.3) then
       ...
       invmmin=2d0*sqrt(pt2)*pb(0)*(1d0-cos(max(drcut,phi)))
       invmmax=sqrts**2
...
!!$       if (p(0,next)**2.lt.pt2) then
!!$          jac=-1d0
!!$          return
!!$       endif
       p(3,next)=sqrt(p(0,next)**2-pt2)
    ...
    elseif (use_mode.eq.4) then
       ...
       costhetamin= abs(p(1,next))/(dsqrt(pt2+pzmax**2))
       costhetamax= cos(max(abs(phi2),drcut))

       if (abs(phi2).ge.pi/2d0) then
          costhetamin= cos(max(abs(phi2),drcut))
          costhetamax = cos(pi-acos(abs(p(1,next))/(dsqrt(pt2+pzmax**2))))
       endif
       ...
       if (costheta.lt.0d0) then
          ...
          p(3,next) = dsqrt((p(1,next)**2/costheta**2)-pt2)
       else
          theta=acos(costheta)
       ...
       !jac = jac*p(1,next)/sqrt(pt2)*0.5d0/(sin(2d0*atan(exp(-ymax)))**2)/((1d0+tan)**1.5d0)
       !jac = jac*cosh(y)
       call boostz(p(0,next),-y,pb)
       phi=atan(p(2,next-1)/p(1,next-1))
       if(p(1,next-1).lt.0d0) phi=phi+pi
