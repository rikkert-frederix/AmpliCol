...
// ---------------------------------------------------------------------------
// External amplitude library (CUDA backend)
//
// `evaluate_amp` here evaluates a whole batch of N events in a single call
// (one GPU kernel launch handles the H2D copy, dispatch per-event on
// ichan[i]/iint[i], and the D2H copy back). AC_D_FP/AC_D_CX/AC_NEXT/AC_NAMP
// and the `evaluate_amp` declaration come from Library/amplib.cuh.
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// Translation of the Fortran `umami_impl` module, unnamed namespace
// (batched-CUDA analogue of umamic.cpp)
// ---------------------------------------------------------------------------
...
// Max amplitudes across all (ichan,iint) pairs, from the metadata file.
// `evaluate_amp` writes AC_NAMP values per event; the two must agree.
...
// One event (N=1) per (ichan,iint) config; batching is exercised by the
// event loop in umami_matrix_element, not here.
// ------------------------------------------------------------------
...
static void test_library()
{
    for (int ichan = 0; ichan < nchans; ++ichan) {
        ...
        for (int iint = 0; iint < max_iint; ++iint) {
            ...
            std::vector<AC_D_FP> p(4 * static_cast<std::size_t>(n));
            std::vector<AC_D_CX> amps_save(static_cast<std::size_t>(namps));
            // evaluate_amp always writes AC_NAMP values per event regardless of (ichan,iint)
            std::vector<AC_D_CX> amps(static_cast<std::size_t>(AC_NAMP));

            f.read(reinterpret_cast<char*>(p.data()),
                   static_cast<std::streamsize>(sizeof(AC_D_FP) * p.size()));
            f.read(reinterpret_cast<char*>(amps_save.data()),
                   static_cast<std::streamsize>(sizeof(AC_D_CX) * namps));

            int ichan1 = ichan + 1;
            int iint1  = iint  + 1;
            evaluate_amp(&ichan1, &iint1, p.data(), amps.data(), 1);
            ...
            for (int i = 0; i < namps; ++i) {
                double denom = cuda::std::abs(amps_save[i]) + cuda::std::abs(amps[i]);
                if (denom > abs_floor &&
                    cuda::std::abs(amps_save[i] - amps[i]) / denom > tol)
                {
                    fprintf(stderr,
...
// ------------------------------------------------------------------
// umami_postprocess_event  (was umami_matrix_element_cxx / umami_matrix_element_impl)
//
// Batched-CUDA analogue: unlike the CPU version, this does NOT call
// evaluate_amp itself. The caller evaluates a whole batch of events with a
// single evaluate_amp(..., N) call and then invokes this per event, passing
// the AC_NAMP amplitudes already computed for that event.
// ------------------------------------------------------------------

static void umami_postprocess_event(
    int             ichan1,     // 1-based
    int             iint1,      // 1-based
    const AC_D_CX*  amps,       // AC_NAMP values for this event
    double          alphas,
    ...
    if (keep_processes_separate) {
        ...
        for (int ih = 0; ih < namps_cur; ++ih) {
            ...
            amp2_hel[ih] = cuda::std::real(amps[ih] * cg.col_fac[iint0] *
                                           cuda::std::conj(amps[ih])) * hf;
        }
        amp2[0] = 0.0;
        for (int ih = 0; ih < namps_cur; ++ih)
            amp2[0] += amp2_hel[ih];
        ...
        for (int ih = 0; ih < cg.namps[0]; ++ih) {
            ...
            amp2_hel[ih] = cuda::std::real(amps[ih] * cg.col_fac[iproc] *
                                           cuda::std::conj(amps[ih])) * hf;
...
struct InterfaceInstance {
    unsigned int magic;
    int          particle_count;
    // No persistent momentum buffer: umami_matrix_element builds a fresh
    // per-call batch (size = count events) since that batch is what gets
    // handed to evaluate_amp(..., N) in a single GPU call.
};
...
UmamiStatus umami_get_meta(UmamiMetaKey meta_key, void* result)
{
    switch (meta_key) {
        case UMAMI_META_DEVICE:
            *static_cast<int*>(result) = UMAMI_DEVICE_CUDA;
            break;
...
UmamiStatus umami_initialize(UmamiHandle* handle, char const* /*param_card_path*/)
{
    ...
    if (!data_loaded) {
        try {
            ...
            // evaluate_amp's per-event output stride is the compile-time
            // AC_NAMP; the metadata-derived max must fit within it.
            if (max_amps_global > AC_NAMP) {
                fprintf(stderr,
                        "amplitude count %d exceeds AC_NAMP=%d compiled into the "
                        "CUDA library; library/metadata mismatch\n",
                        max_amps_global, AC_NAMP);
                pthread_mutex_unlock(&init_mutex);
                return UMAMI_ERROR;
            }
            // The batched kernel packs momenta assuming a single fixed
            // particle count AC_NEXT for every channel.
            if (nchans > 0 && chans[0].n != AC_NEXT) {
                fprintf(stderr,
                        "particle count %d from amplitudes_light.bin does not "
                        "match AC_NEXT=%d compiled into the CUDA library\n",
                        chans[0].n, AC_NEXT);
                pthread_mutex_unlock(&init_mutex);
                return UMAMI_ERROR;
            }
...
UmamiStatus umami_matrix_element(
    ...
    const int n_part = inst->particle_count;

    // Build the whole batch up front so all `count` events can be evaluated
    // in a single evaluate_amp(..., N) call (one GPU kernel launch), rather
    // than one event at a time.
    std::vector<int>     ichan_batch(count);
    std::vector<int>     iint_batch(count);
    std::vector<AC_D_FP> p_batch(static_cast<std::size_t>(count) * 4 * n_part);
    std::vector<double>  alphas_batch(count);
    std::vector<double>  rnd_batch(count);

    for (size_t i = 0; i < count; ++i) {
        ichan_batch[i]  = channel_indices_in ? static_cast<int>(channel_indices_in[i + offset]) + 1 : 1;
        iint_batch[i]   = flavor_indices_in  ? static_cast<int>(flavor_indices_in [i + offset]) + 1 : 1;
        alphas_batch[i] = alpha_s_in         ? alpha_s_in        [i + offset] : 0.118;
        rnd_batch[i]    = random_helicity_in ? random_helicity_in[i + offset] : 0.5;

        // Repack momenta from caller layout into column-major (0:3, n) layout,
        // one event-sized block per event in the batch.
        AC_D_FP* p_event = &p_batch[static_cast<std::size_t>(i) * 4 * n_part];
        for (int j = 0; j < n_part; ++j)
            for (int k = 0; k < 4; ++k)
                p_event[k + 4 * j] =
                    momenta_in[i + offset + stride * (n_part * k + j)];
    }

    std::vector<AC_D_CX> amps_batch(static_cast<std::size_t>(count) * AC_NAMP);
    evaluate_amp(ichan_batch.data(), iint_batch.data(), p_batch.data(),
                 amps_batch.data(), static_cast<int>(count));

    for (size_t i = 0; i < count; ++i) {
        double amp2     = 0.0;
        int    helicity = 0;
        double rnd      = rnd_batch[i];

        umami_postprocess_event(
            ichan_batch[i],
            iint_batch[i],
            &amps_batch[static_cast<std::size_t>(i) * AC_NAMP],
            alphas_batch[i],
            &rnd,
            &amp2,
            &helicity
