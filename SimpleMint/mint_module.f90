...
! tried many points already. Need to crash. 
          write (*,*) 'ERROR: NOT ENOUGH POINTS PASS THE CUTS. ' // &
               'RESULTS CANNOT BE TRUSTED. ',ntotcalls(1),pass_cuts_point
          stop 1
       else
          enough_points=.false.
          return
...
! convert 'flat x' ('rand') to 'vegas x' ('x') and include jacobian ('vol')
  ...
  subroutine write_results
    ...
    if (fixed_order) then
       ...
       open(unit=58,file='results'//trim(adjustl(tag))//'.dat',status='unknown')
       do kchan=0,nchans
       ...
       if (imode.eq.0) then
          open(unit=58,file='res'//trim(adjustl(tag)),status='unknown')
...
! Write the MINT integration grids to file
    implicit none
    integer :: i,j,k,kchan
    open (unit=12,file='mint_grids'//trim(adjustl(tag)),status='unknown')
    do kchan=1,nchans
       do j=0,nintervals
...
! Read the MINT integration grids from file
    ...
    open (unit=12, file='mint_grids'//trim(adjustl(tag_read)),status='old')
    ans(1,0)=0d0
    unc(1,0)=0d0
    do kchan=1,nchans
       do j=0,nintervals
...
!     pick one at random (weighted by cross section)
  ...
  subroutine close_run_zero_res
    ...
    call write_grids_to_file
    call write_results
    open (unit=12, file='res'//trim(adjustl(tag))//'.dat',status='unknown')
    do kchan=0,nchans
