...
! Write the MINT integration grids to file
    implicit none
    integer :: i,j,k,kchan
    open (unit=12,file='Outputs'//trim(adjustl(add_arg))//'/Res_files/mint_grids'//trim(adjustl(tag)),status='unknown')
    do kchan=1,nchans
       do j=0,nintervals
...
!     pick one at random (weighted by cross section)
  ...
  subroutine close_run_zero_res
    ...
    call write_grids_to_file
    call write_results
    open (unit=12, file='Outputs'//trim(adjustl(add_arg))//'/Res_files/res'//trim(adjustl(tag))//'.dat',status='unknown')
    do kchan=0,nchans
