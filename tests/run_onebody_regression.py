...
def process_summary(path: Path) -> tuple[tuple[int, ...], int]:
    ...
    n_external, n_unique = (int(value) for value in lines[0][:2])
    if n_external != 3 or n_unique != 1:
        raise AssertionError(f"unexpected one-body process header: {lines[0]}")
    process = tuple(int(value) for value in lines[1])
    groups = int(lines[2][0])
