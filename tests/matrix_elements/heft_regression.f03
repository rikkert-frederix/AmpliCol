program heft_regression
  ...
  call check_mixed_catalogue_keeps_gluon_heft()
...
contains
  ...
  subroutine check_mixed_catalogue_keeps_gluon_heft()
    ! The unique-process probe evaluates gluon- and quark-initiated rows in
    ! one amplitude object.  Quark-line ordering constraints must not remove
    ! a pure-gluon HEFT row merely because a quark row is also present.
    implicit none
    integer,parameter :: n=4
    type(amplitude_QCD) :: gluon_only,mixed
    integer,dimension(n,1) :: gluon_part,gluon_order
    integer,dimension(n,2) :: mixed_part,mixed_order
    integer,dimension(0:3,n) :: spin
    integer,dimension(n) :: hel
    real(kind=dp),dimension(0:3,n) :: p
    real(kind=dp) :: pure_norm,mixed_norm
    integer :: first,last

    gluon_part(:,1)=[21,21,21,25]
    gluon_order(:,1)=[4,1,2,3]
    mixed_part(:,1)=gluon_part(:,1)
    mixed_part(:,2)=[-1,1,21,25]
    mixed_order(:,1)=gluon_order(:,1)
    mixed_order(:,2)=[1,3,4,2]
    spin=0
    spin(0,1:3)=2
    spin(1,1:3)=-1
    spin(2,1:3)=1
    ...
    call gluon_only%init(1,n,1,gluon_part,spin,gluon_order,model)
    call gluon_only%evaluate(n,p,hel,.false.,model,gs,gew,gheft)
    call mixed%init(1,n,2,mixed_part,spin,mixed_order,model)
    call mixed%evaluate(n,p,hel,.false.,model,gs,gew,gheft)

    first=mixed%iproc_start(1)
    last=mixed%iproc_start(2)-1
    if (last-first+1.ne.gluon_only%n_amps) then
       write (*,*) 'Mixed HEFT catalogue dropped gluon amplitudes:',&
            last-first+1,gluon_only%n_amps
       stop 1
    endif
    pure_norm=sum(abs(gluon_only%amps)**2)
    mixed_norm=sum(abs(mixed%amps(first:last))**2)
    call assert_real_close('mixed-catalogue gg > gh',mixed_norm,pure_norm,1d-13)
  end subroutine check_mixed_catalogue_keeps_gluon_heft
