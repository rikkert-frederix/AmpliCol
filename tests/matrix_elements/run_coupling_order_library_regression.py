#!/usr/bin/env python3
"""Build and run the generated multi-root coupling-sector library check."""
...
def main() -> None:
    ...
    verifier = root / "tests/matrix_elements/coupling_order_library_regression.f03"
    with tempfile.TemporaryDirectory(prefix="amplicol-coupling-order-library-") as tmp_name:
        tmp = Path(tmp_name)
        (tmp / "Library").mkdir()
        subprocess.run([str(generator), "--emit-library"], cwd=tmp, check=True)

        generated = tmp / "Library/amp1_1_lib.f03"
        checker = tmp / "coupling_order_library_regression"
        command = [
