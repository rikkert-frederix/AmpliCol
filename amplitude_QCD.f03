...
!!$      endif
    ...
    subroutine allocate_and_fill_currents_to_amps_map()
      ...
      ! amplitudes
      implicit none
      integer :: icur,jcur,i,j,iproc,iamp,heft_order,max_candidates,new_capacity
      type(bitset) :: proc
      integer,dimension(:,:),allocatable :: curr2amp,curr2amp_expanded
      integer,dimension(:,:,:),allocatable :: amp_currents,amp_currents_expanded
      ...
      allocate(curr2amp(1:2,1:max(1,max_candidates)))
      allocate(amp_currents(1:2,0:1,1:max(1,max_candidates)))
      curr2amp=0
      amp_currents=0
         ...
         do icur=this%n_cur_start(n-1),this%n_cur_end(n-1)
            do jcur=this%n_cur_start(n),this%n_cur_end(n)
               ...
               if (iamp.gt.this%n_amps) then
                  ! A current pair can occur in several process segments.
                  ! Grow both closure maps together when a shared family
                  ! exceeds the one-pair/one-amplitude initial bound.
                  if (this%n_amps.eq.size(curr2amp,2)) then
                     new_capacity=max(1,2*size(curr2amp,2))
                     allocate(curr2amp_expanded(1:2,1:new_capacity))
                     allocate(amp_currents_expanded(1:2,0:1,1:new_capacity))
                     curr2amp_expanded=0
                     amp_currents_expanded=0
                     curr2amp_expanded(:,1:size(curr2amp,2))=curr2amp
                     amp_currents_expanded(:,:,1:size(amp_currents,3))=amp_currents
                     call move_alloc(curr2amp_expanded,curr2amp)
                     call move_alloc(amp_currents_expanded,amp_currents)
                  endif
...
13   ext_from_cur=order(nc,1,iproc)
    ...
    logical function valid_current_combination()
      ...
      implicit none
      integer :: i,j,nc1,nc2,c,iproc_local
      ...
      if (.not. gluon_current) then
         ...
         if (isize.eq.n-1) then
            do iproc_local=1,this%nprocs
               if (.not.iproc_combined%test_bit(iproc_local)) cycle
               if (this%n_qqbar(iproc_local).gt.0 .and. &
    ...
    logical function valid_three_current_combination(candidate)
      ...
      if (isize.eq.n-1) then
         do iproc_local=1,this%nprocs
            if (.not.common_processes%test_bit(iproc_local)) cycle
            if (this%n_qqbar(iproc_local).gt.0) then
  ...
  subroutine evaluate(this,n,p,hel,read_file,pm,gs,gew,gheft)
    ...
    type is (amplitude_QCD)
       call evaluate_concrete(concrete,n,p,hel,read_file,pm,gs,gew,gheft)
    class default
  ...
  subroutine evaluate_concrete(this,n,p,hel,read_file,pm,gs,gew,gheft)
    use FeynmanRules
    use particles
    implicit none
