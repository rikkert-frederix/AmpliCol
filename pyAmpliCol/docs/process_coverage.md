...
| Colorless and spin-2 models | Implemented and documented | UFO, loader JSON, and compiled-model inputs converge to identical canonical IR for `scalars` and `scalar_gravity`. Scalar contacts are validated through final-state `n=8`, including the direct ten-point vertex and an independent eager n-ary oracle. The uncapped scalar model, including all tree diagrams and internal propagators, is validated in double and 50-digit evaluation through `n=7`; that row has 32,176 currents and 684,889 interactions and completes in 382.0 s under the 30 GB watchdog. Scalar gravity is validated through four final gravitons; that artifact has 378 currents, 2,168 interactions, and 16 roots, and completes in 880.6 s. Unchanged zero-filter warmups are reused by current merging below a 64 MiB cache gate. LC/NLC/full use the same unit singlet metric. |
...
| Process sets with `PROC | PROC` | Implemented in parser/CLI artifacts | Multi-entry generation writes a root `process_set_manifest.json`, one root multi-language `API/` bundle, and nested subprocess artifacts. Crossing aliases and subprocesses do not duplicate the bundle. |
...
| Generic schema-v2 process artifact | Implemented as production artifact | `generic_artifact.py` serializes process IR, colour plan, currents, interactions, closures, physics metadata, Symbolica evaluator stages, and validation points into `pyamplicol-generic-dag-process` artifacts. |
...
| Matrix generation facade | Generic-only | `matrix.py` is a process-generic schema-v2 planning facade. The previous family-shaped `CurrentKey`/`RecursionGraph` implementation has been removed. |
...
| Python artifact loader | Implemented for schema-v2 generic DAG artifacts | `load_process(..., runtime="python")` resolves process sets, subprocess keys, generic external layouts, and stage-plan metadata without depending on process-family labels. Schema-v1 process and process-set manifests are rejected explicitly. |
| Rusticol runtime | Implemented for schema-v2 generic DAG artifacts | `rusticol.Runtime.load()` accepts only schema-v2 generic DAG artifacts. `evaluate()` preserves the optimized summed reduction; `evaluate_resolved()` exposes stable physical helicity/colour IDs and expands exact folded symmetries without rerunning the DAG per component. On AArch64, the f64 path uses native two-lane complex SymJIT payloads and shares one packed stage-local input across output chunks. |
| Multi-language runtime API | Implemented through one shared Rust core | PyO3/NumPy are optional features. C ABI v1, a header-only C++17 wrapper, and a Fortran 2008 `ISO_C_BINDING` module expose metadata, selectors, atomic model updates, summed evaluation, and resolved f64 evaluation without linking Python. Generated roots contain one `API/` example bundle. |
| Compact runnable manifests | Implemented | Large production artifacts omit generation-only current/value metadata and reduce stage interactions to IDs plus vertex-kind counts. Small detailed and plan-only artifacts retain full records; large diagnostic sidecars explicitly identify their compact layout. Rusticol loads both forms. |
...
Rusticol and the Python process facade reject schema-v1 artifacts explicitly.
The schema-v1 writers, runtime modules, alternate loader, and historical tests
have been removed. New process-generation work must target schema-v2 manifests.
The Fortran AmpliCol reference adapter and `processes --legacy-output` are
independent compatibility surfaces and remain supported.
