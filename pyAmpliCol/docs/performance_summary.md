...
The dependency installer now uses upstream Symbolica `dev` plus the local pyAmplicCol patches and pins SymJIT to `2.19.3` at commit `dee77e14c8bf9c8d5304dbda2e393c52ee3e4cd4`. The refreshed staged-DAG JIT rows pass SymJIT `opt_level=3` through Symbolica's JIT evaluator.
